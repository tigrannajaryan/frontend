...
export const upcomingAppointmentsMock = genFake(5, AppointmentStatus.new);
export const lastVisitedAppointmentsMock = genFake(5, AppointmentStatus.checked_out)[0];

export const pastAppointmentsMock = genFake(5, AppointmentStatus.checked_out);
...
export class AppointmentsApiMock {
  ...
  getHome(upcomingCount = 0, lastVisitedCount = 0): Observable<ApiResponse<HomeResponse>> {
    return new Observable(observer => {
      observer.next({
        response: {
          upcoming: upcomingAppointmentsMock,
          last_visited: lastVisitedAppointmentsMock
        }
      });
      observer.complete();
    });
